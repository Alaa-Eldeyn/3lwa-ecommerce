...
const updateAccessAndRefreshTokens = (newAccessToken: string, newRefreshToken: string) => {
  const storedUser = getCookie("basitUser");
  if (storedUser) {
    try {
      const user = JSON.parse(storedUser as string);
...
export const getNewAccessToken = async (refreshToken: string, email: string): Promise<string> => {
  try {
...
customAxios.interceptors.response.use(
  ...
  async (error) => {
    ...
    try {
      ...
      console.log("Failed to get new access token, trying to regenerate refresh token...");
...
// Email-specific axios instance
export const customAxiosEmail = axios.create({
  baseURL: process.env.NEXT_PUBLIC_BASE_URL_EMAIL,
  headers: {
    Accept: "application/json",
    "Content-Type": "application/json",
    "X-Platform": "website",
...
// Request interceptor - لإضافة الـ token للـ headers
customAxiosEmail.interceptors.request.use(
  (config) => {
    const userStr = getCookie("basitUser");
...
// Response interceptor - للتعامل مع 401 errors
customAxiosEmail.interceptors.response.use(
  (response) => response,
  async (error) => {
    const originalRequest = error.config;
    ...
    try {
      ...
      const newAccessToken = await getNewAccessToken(refreshToken, email);
      originalRequest.headers.Authorization = `Bearer ${newAccessToken}`;
      return customAxiosEmail(originalRequest);
    } catch (accessTokenError) {
      console.log("Failed to get new access token, trying to regenerate refresh token...");
