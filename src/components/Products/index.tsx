...
const Products = () => {
  ...
  // Fetch dynamic filters based on categoryId (moved up to use in priceRange initialization)
  const { data: filtersData } = useQuery({
    queryKey: ["dynamicFilters", categoryIdFromUrl, searchTerm],
    queryFn: async () => {
  ...
  // Initialize priceRange with lazy initialization
  ...
  const [verifiedVendorsOnly, setVerifiedVendorsOnly] =
  ...
  // Single query for products with all filters
  const { data: productsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (hasActiveFilters) {
        ...
        if (selectedColors.length > 0) payload.colors = selectedColors;
        if (selectedSizes.length > 0) payload.sizes = selectedSizes;
        if (minItemRating > 0) payload.minItemRating = minItemRating;
        if (minVendorRating > 0) payload.minVendorRating = minVendorRating;
        if (inStockOnly) payload.inStockOnly = inStockOnly;
        if (freeShippingOnly) payload.freeShippingOnly = freeShippingOnly;
        if (verifiedVendorsOnly) payload.verifiedVendorsOnly = verifiedVendorsOnly;
        if (primeVendorsOnly) payload.primeVendorsOnly = primeVendorsOnly;
        if (onSaleOnly) payload.onSaleOnly = onSaleOnly;
  ...
  // Compute initial price range from filtersData
